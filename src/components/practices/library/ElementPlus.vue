...
const confirmDelete = () => {
  ElMessageBox.confirm('서버에서 해당 파일을 영구히 삭제하시겠습니까?', '최종 확인', {
...
const startDownload = () => {
  if (isDownloading.value) return // 진행 중일 땐 중복 실행 막기
  ...
  const interval = setInterval(() => {
    ...
    if (downloadProgress.value >= 100) {
      ...
      ElMessage.success('데이터 로드가 완료되었습니다!')
    }
  }, 400)
}
</script>
...
<template>
  <div class="practice-section">
    ...
    <p class="subtitle">외부 API 없이 컴포넌트와 상태값 변동만으로 동작을 확인하는 샘플입니다.</p>
      ...
      <div class="card-body">
        ...
        <div class="input-group">
          <el-switch
            ...
            active-text="개인정보 수집 및 이용약관에 동의합니다."
          />
        </div>
    ...
    <el-card class="box-card">
      <template #header><strong>🛒 실습 2. 상품 수량 및 평점</strong></template>
      <div class="card-body">
        <div class="input-group">
          <span>구매 수량 선택:</span>
          <el-input-number v-model="productQuantity" :min="1" :max="10" />
          <span class="hint-text">(최대 10개)</span>
        </div>
        ...
        <div class="result-preview">
          선택 수량 <strong>{{ productQuantity }}개</strong> / 준 점수
    ...
    <el-card class="box-card">
      <template #header><strong>⚙️ 실습 3. 피드백 &amp; 프로그레스</strong></template>
      <div class="card-body">
        <div class="btn-zone">
